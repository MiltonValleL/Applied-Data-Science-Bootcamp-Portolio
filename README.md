...
| Project | Type | Key Challenge | Result / Metric | Complexity |
| :--- | :--- | :--- | :--- | :--- |
| **[Megaline's Tariff Plan Recommendations](./Megaline's_Tariff_Plan_Recommendations(Classification))** | Classification (Model Selection) | Comparing Decision Trees vs. Random Forests for accuracy optimization. | **Accuracy: 80.4%** (Target: ≥0.75) | Easy Level|
| **[Customer Churn Prediction for Beta Bank (Easy Level)](./Customer_Churn_Prediction_for_Beta_Bank)** | Classification (Imbalanced Data) | Handling severe class imbalance using Upsampling & Class Weighting. | **F1 Score: 0.61** (Target: ≥0.59) | Easy Level|
| **[Time Series Forecasting for Taxi Orders (Easy Level)](./Time_Series_Forecasting_for_Taxi_Orders)** | Time Series Regression | Predicting demand peaks using `LightGBM` and time-based features. | **RMSE: 45.4** (Target: ≤48) | Easy Level|
| **[Customer Churn Prediction (Final Project)](./Customer_Churn_Prediction-(Final_Project))** | Classification (Gradient Boosting) | Integrating multiple data sources & maximizing AUC-ROC. | **AUC-ROC: 0.86** (Target: ≥0.81) | Mid Level |
